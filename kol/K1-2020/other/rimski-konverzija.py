...
# pretvorit sve rimske u decimalni
# zbrojit sve redove
# pretvorit u decimalni ili rimski, ovisno koji ima manje znakova
...
def roman_to_decimal(roman):
    return sum([roman_dict[char] for char in roman])
...
def sum_rows_decimal(redovi):
    brojevi = [roman_to_decimal(red) if not is_number(red)
               else int(red) for red in redovi]
    return sum(brojevi)


def calc_min_chars(redovi):
    suma_redova = sum_rows_decimal(redovi)
    print('suma redova', suma_redova)
    suma_as_roman = decimal_to_roman(suma_redova)
    return (suma_as_roman, len(suma_as_roman))


# print(roman_to_decimal('XVIII'))
